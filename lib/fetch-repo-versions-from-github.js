...
module.exports = function fetchRepoVersionsFromGithub(repo, options) {
  ...
  if (!repo) {
    throw new Error('A repository must be specified');
  }
  ...
  return get('https://api.github.com/repos/components/' + repo + '/tags?' + params, { timeout: 1000 })
    .then(function(res) {
      return res.json();
    }).then(function(json) {
      ...
      if (json.length === perPage) {
        return fetchRepoVersionsFromGithub(repo, {
