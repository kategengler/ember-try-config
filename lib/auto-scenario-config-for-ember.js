...
module.exports = function generateConfig(options) {
  return generateScenariosFromSemver(options.versionCompatibility, options.availableVersions).then(function(scenarios) {
    return { scenarios:  [].concat(baseScenarios(options.versionCompatibility.includeDependencies), scenarios) };
  });
};

function generateScenariosFromSemver(semverStatements, availableVersions) {
  var includeDependencies = semverStatements.includeDependencies;
...
function getDependencies(source, dependencies) {
  return included[source].reduce(function(obj, d) {
    if(dependencies[d]) {
      obj[d] = dependencies[d];
    }
...
function baseScenarios(includeDependencies) {
  var beta, canary;
